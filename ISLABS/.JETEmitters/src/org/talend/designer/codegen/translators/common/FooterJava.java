...
public class FooterJava
{
  ...
  protected final String TEXT_3 = NL + "    // ESB Service Activity Monitor Feature" + NL + "    private org.apache.cxf.feature.AbstractFeature eventFeature;" + NL + "" + NL + "    public void setEventFeature(org.apache.cxf.feature.AbstractFeature eventFeature) {" + NL + "        this.eventFeature = eventFeature;" + NL + "    }";
  ...
  protected final String TEXT_15 = NL + NL + "        try {" + NL + "            //call job/subjob with an existing context, like: --context=production. if without this parameter, there will use the default context instead." + NL + "            java.io.InputStream inContext = ";
  protected final String TEXT_16 = ".class.getClassLoader().getResourceAsStream(\"";
  protected final String TEXT_17 = "/";
  protected final String TEXT_18 = "/contexts/\"+contextStr+\".properties\");" + NL + "            if(isDefaultContext && inContext ==null) {" + NL + "" + NL + "            } else {" + NL + "                if (inContext!=null) {" + NL + "                    //defaultProps is in order to keep the original context value" + NL + "                    defaultProps.load(inContext);" + NL + "                    inContext.close();" + NL + "                    context = new ContextProperties(defaultProps);" + NL + "                }else{" + NL + "                    //print info and job continue to run, for case: context_param is not empty." + NL + "                    System.err.println(\"Could not find the context \" + contextStr);" + NL + "                }" + NL + "            }" + NL + "" + NL + "            if(!context_param.isEmpty()) {" + NL + "                context.putAll(context_param);" + NL + "            }";
  protected final String TEXT_19 = NL + "            try{" + NL + "                String context_";
  protected final String TEXT_20 = "_value = context.getProperty(\"";
  protected final String TEXT_21 = "\");" + NL + "                if (context_";
  protected final String TEXT_22 = "_value == null){" + NL + "                    context_";
  protected final String TEXT_23 = "_value = \"\";" + NL + "                }" + NL + "                int context_";
  protected final String TEXT_24 = "_pos = context_";
  protected final String TEXT_25 = "_value.indexOf(\";\");" + NL + "                String context_";
  protected final String TEXT_26 = "_pattern =  \"yyyy-MM-dd HH:mm:ss\";" + NL + "                if(context_";
  protected final String TEXT_27 = "_pos > -1){" + NL + "                    context_";
  protected final String TEXT_28 = "_pattern = context_";
  protected final String TEXT_29 = "_value.substring(0, context_";
  protected final String TEXT_30 = "_pos);" + NL + "                    context_";
  protected final String TEXT_31 = "_value = context_";
  protected final String TEXT_32 = "_value.substring(context_";
  protected final String TEXT_33 = "_pos + 1);" + NL + "                }" + NL + "" + NL + "                context.";
  protected final String TEXT_34 = "=(java.util.Date)(new java.text.SimpleDateFormat(context_";
  protected final String TEXT_35 = "_pattern).parse(context_";
  protected final String TEXT_36 = "_value));" + NL + "" + NL + "            }catch(ParseException e)" + NL + "            {" + NL + "                context.";
  protected final String TEXT_37 = "=null;" + NL + "            }";
  protected final String TEXT_38 = NL + "                context.";
  protected final String TEXT_39 = "=(";
  protected final String TEXT_40 = ") context.getProperty(\"";
  protected final String TEXT_41 = "\");";
  protected final String TEXT_42 = NL + "             context.";
  protected final String TEXT_43 = "= new java.text.StringCharacterIterator(context.getProperty(\"";
  protected final String TEXT_44 = "\")).first();";
  protected final String TEXT_45 = NL + "             try{" + NL + "                 context.";
  protected final String TEXT_46 = "=routines.system.ParserUtils.parseTo_";
  protected final String TEXT_47 = " (context.getProperty(\"";
  protected final String TEXT_48 = "\"));" + NL + "             }catch(NumberFormatException e){" + NL + "                 context.";
  protected final String TEXT_49 = "=null;" + NL + "              }";
  protected final String TEXT_50 = NL + "        } catch (java.io.IOException ie) {" + NL + "            System.err.println(\"Could not load context \"+contextStr);" + NL + "            ie.printStackTrace();" + NL + "        }" + NL + "" + NL + "" + NL + "        // get context value from parent directly" + NL + "        if (parentContextMap != null && !parentContextMap.isEmpty()) {";
  protected final String TEXT_51 = "if (parentContextMap.containsKey(\"";
  protected final String TEXT_52 = "\")) {" + NL + "                context.";
  protected final String TEXT_53 = " = (";
  protected final String TEXT_54 = ") parentContextMap.get(\"";
  protected final String TEXT_55 = "\");" + NL + "            }";
  protected final String TEXT_56 = NL + "        }" + NL + "" + NL + "        //Resume: init the resumeUtil" + NL + "        resumeEntryMethodName = ResumeUtil.getResumeEntryMethodName(resuming_checkpoint_path);" + NL + "        resumeUtil = new ResumeUtil(resuming_logs_dir_path, isChildJob, rootPid);" + NL + "        resumeUtil.initCommonInfo(pid, rootPid, fatherPid, projectName, jobName, contextStr, jobVersion);" + NL + "" + NL + "        //Resume: jobStart" + NL + "        resumeUtil.addLog(\"JOB_STARTED\", \"JOB:\" + jobName, parent_part_launcher, Thread.currentThread().getId() + \"\", \"\",\"\",\"\",\"\",resumeUtil.convertToJsonText(context));" + NL;
  ...
  protected final String TEXT_59 = NL + "    try {" + NL + "        runTrace.openSocket(!isChildJob);" + NL + "        runTrace.startThreadTrace(clientHost, portTraces);" + NL + "    } catch (java.io.IOException ioException) {" + NL + "        ioException.printStackTrace();" + NL + "    }";
  protected final String TEXT_60 = NL;
  protected final String TEXT_61 = NL;
  protected final String TEXT_62 = NL + NL + "    java.util.concurrent.ConcurrentHashMap<Object, Object> concurrentHashMap = new java.util.concurrent.ConcurrentHashMap<Object, Object>();" + NL + "    globalMap.put(\"concurrentHashMap\", concurrentHashMap);" + NL + "" + NL + "    long startUsedMemory = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();" + NL + "    long endUsedMemory = 0;" + NL + "    long end = 0;" + NL + "" + NL + "    startTime = System.currentTimeMillis();";
  protected final String TEXT_63 = NL + "        ";
  protected final String TEXT_64 = ".addMessage(\"begin\");";
  protected final String TEXT_65 = NL;
  protected final String TEXT_66 = NL;
  protected final String TEXT_67 = NL + NL + "this.globalResumeTicket = true;//to run tPreJob" + NL;
  protected final String TEXT_68 = NL;
  protected final String TEXT_69 = NL + NL;
  protected final String TEXT_70 = NL + "        try {";
  protected final String TEXT_71 = NL + "            ";
  protected final String TEXT_72 = "Process(globalMap);" + NL + "        } catch (java.lang.Exception e) {" + NL + "            e.printStackTrace();" + NL + "        }";
  protected final String TEXT_73 = NL + NL + "this.globalResumeTicket = false;//to run others jobs" + NL;
  protected final String TEXT_74 = NL + "        runningThreadCount.add(1);" + NL + "        new Thread(){" + NL + "            public void run() {" + NL + "                java.util.Map threadRunResultMap = new java.util.HashMap();" + NL + "                threadRunResultMap.put(\"errorCode\", null);" + NL + "                threadRunResultMap.put(\"status\", \"\");" + NL + "                threadLocal.set(threadRunResultMap);" + NL;
  protected final String TEXT_75 = NL + "                ";
  protected final String TEXT_76 = NL + "                finally {" + NL + "                    Integer localErrorCode = (Integer)(((java.util.Map)threadLocal.get()).get(\"errorCode\"));" + NL + "                    String localStatus = (String)(((java.util.Map)threadLocal.get()).get(\"status\"));" + NL + "                    if (localErrorCode != null) {" + NL + "                        if (errorCode == null || localErrorCode.compareTo(errorCode) > 0) {" + NL + "                           errorCode = localErrorCode;" + NL + "                        }" + NL + "                    }" + NL + "                    if (!status.equals(\"failure\")){" + NL + "                        status = localStatus;" + NL + "                    }" + NL + "" + NL + "                    runningThreadCount.add(-1);" + NL + "                }" + NL + "            }" + NL + "        }.start();" + NL;
  protected final String TEXT_77 = NL + "    while (runningThreadCount.getCount() > 0) {" + NL + "        try {" + NL + "            Thread.sleep(10);" + NL + "        } catch (java.lang.Exception e) {" + NL + "            e.printStackTrace();" + NL + "        }" + NL + "    }";
